...
tic
  ...
  [vidWidth1, vidHeight1, nRGB1, vidNFrames1] = size(vidFrames3_1);
  [vidWidth2, vidHeight2, nRGB2, vidNFrames2] = size(vidFrames3_2);
  FrameRate = 60;
  ...
  for k = 1:vidNFrames1
    for i = 1:nRGB1
      for j = 1:vidHeight1
        vidFrames3_1T(j,:,i,k) = vidFrames3_1(:,j,i,k);
      end
    end
    mov1(k).cdata = vidFrames3_1T(:,:,:,k);
  end
  for k = 1:vidNFrames2
    mov2(k).cdata = vidFrames3_2(:,:,:,k);
  end
toc
...
tic
  ...
  for i = 1:M
    Mode_Background(i,:) = mode(Data1(i,:));
  end
...
for i = 1:vidNFrames1
  ...
  if(i == 1)
    Frame_Analysis(:,1:250) = 0;
    Frame_Analysis(:,320:vidWidth1) = 0;
    Frame_Analysis(1:300,:) = 0;
    Frame_Analysis(400:vidHeight1,:) = 0;
  end
